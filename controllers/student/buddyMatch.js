...
const findStudyBuddy = async(currentUserId)=>{
  const currentUser = await User.findById(currentUserId);
  const { studyPrefrences } = currentUser
  const {goals, availability, subjects} = studyPrefrences;

  const candidates = await User.find({
    _id : {$ne : currentUserId},
    buddies : { $ne : currentUserId},
    'studyPrefrences.subjects' : {$in : subjects},
    'studyPrefrences.availability' : {$in : availability}
  });
